...
SELECT `students`.`id` ,`students`.`name` AS `nome_studente`, `students`.`surname`, `courses`.`name` AS `nome_corso`, `courses`.`description`, COUNT(`exam_student`.`exam_id`) as Tentativi_Esame, MAX(`exam_student`.`vote`)
FROM `students` 
JOIN `exam_student` 
